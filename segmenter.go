...
func (seg *Segmenter) AddDictionary(dict Dict) {
	if seg.dict == nil {
		seg.dict = NewDictionary()
	}
	words := splitTextToWords([]byte(dict.Txt))
	newToken := Token{text: words, frequency: dict.Frequency, pos: dict.Pos, ID: dict.ID}
	seg.dict.addToken(newToken)
	seg.Frequency()
}

func (seg *Segmenter) DelDictionary(dict Dict) {
	if seg.dict == nil {
		return
	}
	words := splitTextToWords([]byte(dict.Txt))
	newToken := Token{text: words, frequency: dict.Frequency, pos: dict.Pos, ID: dict.ID}
	seg.dict.delToken(newToken)
	seg.Frequency()
}
...
func (seg *Segmenter) LoadExtendDictionary(list []*Dict) {
	...
	for _, val := range list {
		words := splitTextToWords([]byte(val.Txt))
		token := Token{text: words, frequency: val.Frequency, pos: val.Pos, ID: val.ID}
		seg.dict.addToken(token)
	}
	seg.Frequency()
	log.Println("sego词典载入完毕")
...
func (seg *Segmenter) SortDictionary() {
	if seg.dict == nil {
		return
	}
	// 对每个分词进行细致划分，用于搜索引擎模式，该模式用法见Token结构体的注释。
